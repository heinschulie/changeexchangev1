...
!function(){"use strict";var t=angular.module("cxcApp"),e=function(t,e,a,o,s,n,r,c){e.changeAgentPost=!1,e.sameAuthorPosts=[],e.posturl=a.absUrl(),e.sameCategoryPosts=[],e.recommendedMessageCat="",e.recommendedMessageAuthor="",e.contentState=c,e.contentState.data.menuActive=!1,e.contentState.data.galleryViewing=!1,e.socialState=r;var g=o.slug;e.callForPost=function(){return c.getPostByTitle(g).then(function(t){e.recommendedMessageCat="Enjoyed that article? Then we suggest you try one of these below...",e.recommendedMessageAuthor="More articles from "+c.data.post.author.name})};var u=function(){c.getPostsByAuthor().then(function(t){e.sameAuthorPosts=t})},l=function(){e.sameAuthorPosts=c.data.posts.filter(function(t){return t.author.username===c.data.post.author.username&&t.ID!==c.data.post.ID}),e.sameAuthorPosts.length<3&&u(),e.sameCategoryPosts=c.data.posts.filter(function(t){if(t.ID===c.data.post.ID)return!1;for(var e=t.terms.category.map(function(t){return t.name}),a=0;a<e.length;a++)for(var o=0;o<c.data.categories.length;o++)if(e[a]===c.data.categories[o]&&"Change Moments"!==e[a]&&"Exchange"!==e[a])return!0})};g===c.data.post.ID?("Change agents"===c.data.categories[0]&&(e.changeAgentPost=!0),c.data.posts.length<1&&c.getPosts(),l()):e.callForPost().then(function(t){e.myModel={Url:""+a.absUrl(),Name:c.data.post.title,ImageUrl:c.data.post.featured_image.source},"Change agents"===c.data.categories[0]&&(e.changeAgentPost=!0),c.data.posts.length<1?c.getPosts().then(l):l()})};t.controller("postController",["$rootScope","$scope","$location","$routeParams","$sce","$timeout","socialState","contentState",e])}();
